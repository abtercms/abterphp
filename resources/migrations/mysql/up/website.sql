...
INSERT INTO `casbin_rule` (`ptype`,`v0`,`v1`,`v2`) VALUES ('p','admin','pages','advanced-write'),('p','admin','blocks','advanced-write'),('p','content-editor','pages','advanced-write'),('p','content-editor','blocks','advanced-write'),('p','layout-editor','pages','advanced-write'),('p','layout-editor','blocks','advanced-write');

--
-- Table data for table `user_groups`
--

INSERT INTO `user_groups` (`identifier`, `name`) VALUES ('content-editor','Content Editor'),('layout-editor','Layout Editor');
...
INSERT INTO `user_groups_pages` (`user_group_id`, `page_id`)
SELECT `user_groups`.`id` AS `user_group_id`, `pages`.`id` AS `file_category_id`
FROM `user_groups`
...
FROM user_groups INNER JOIN admin_resources ON admin_resources.identifier IN ('blocklayouts','pagelayouts','pages','blocks')
WHERE user_groups.identifier = 'layout-editor';

-- Provide access to relevant pages for content editors
INSERT IGNORE INTO `user_groups_admin_resources` (`user_group_id`, `admin_resource_id`)
SELECT user_groups.id AS user_group_id, admin_resources.id AS admin_resource_id
FROM user_groups INNER JOIN admin_resources ON admin_resources.identifier IN ('pages','blocks')
