...
{
    /**
     * @param array<string,string|string[]|Attribute>|null $values
     *
    ...
    public static function fromArray(?array $values): array
    {
        ...
        foreach ($values as $key => $value) {
            if ($value instanceof Attribute) {
                assert(
                    $value->getKey() == $key,
                    Debug::prettyPrint('array key does not match attribute key: ', $key, $value->getKey())
                );
     ...
     * @return array
     */
    public static function addItem(array $attributes, string $key, ...$values): array
    {
        $attributes[$key] = new Attribute($key, ...$values);
     ...
     * @return bool
     */
    public static function isValid(array $attributes): bool
    {
        if (!Collection::allInstanceOf($attributes, Attribute::class)) {
            return false;
        }
