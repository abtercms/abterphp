...
abstract class Filter extends Tag implements IFilter, ITemplater
{
    ...
    public function __construct(
        ...
        $fixedAttributes = Attributes::fromArray(
            [
        ...
        $this->attributes = Attributes::merge($this->attributes, $fixedAttributes);

        $this->wrapper = new Tag(null, [], [], Html5::TAG_DIV);

        $this->label = new Label($inputName);
        $this->label->setContent($fieldName);

        $this->helpBlock = new Tag(static::HELP_CONTENT, [static::INTENT_HELP_BLOCK], [], Html5::TAG_P);
    }
