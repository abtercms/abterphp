...
{
    protected const JS_ONLY_WARNING_CLASS = 'only-js-form-warning';

    protected const USER_NAME_ATTRIBS           = [Html5::ATTR_NAME => [Input::AUTOCOMPLETE_OFF]];
    protected const JS_ONLY_ATTRIBS             = [Html5::ATTR_CLASS => self::JS_ONLY_WARNING_CLASS];
    protected const FORM_GROUP_REQUIRED_ATTRIBS = [Html5::ATTR_CLASS => FormGroup::CLASS_REQUIRED];
    protected const EMAIL_ATTRIBS               = [
        Html5::ATTR_AUTOCOMPLETE => [Input::AUTOCOMPLETE_OFF],
        Html5::ATTR_TYPE         => Input::TYPE_EMAIL,
    ];
    protected const RAW_PASSWORD_ATTRIBS        = [
        Html5::ATTR_NAME => [Input::AUTOCOMPLETE_OFF],
        Html5::ATTR_TYPE => [Input::TYPE_PASSWORD],
    ];
    protected const PASSWORD_ATTRIBS            = [Html5::ATTR_TYPE => [Input::TYPE_HIDDEN]];
    protected const PASSWORD_CONFIRMED_ATTRIBS  = [Html5::ATTR_TYPE => [Input::TYPE_HIDDEN]];
    ...
    protected function addJsOnly(): User
    {
        ...
        $this->form[] = new Tag($content, [], static::JS_ONLY_ATTRIBS, Html5::TAG_P);
    ...
    {
        $input = new Input('username', 'username', $entity->getUsername(), [], static::USER_NAME_ATTRIBS);
        $label = new Label('username', 'admin:userUsername');

        $this->form[] = new FormGroup($input, $label, null, [], static::FORM_GROUP_REQUIRED_ATTRIBS);
    ...
    {
        $input = new Input('email', 'email', $entity->getEmail(), [], static::EMAIL_ATTRIBS);
    ...
    {
        $this->form[] = new Input('password', 'password', '', [], static::PASSWORD_ATTRIBS);
    ...
    protected function addPasswordConfirmed(): User
    {
        $this->form[] = new Input(
            ...
            '',
            [],
            static::PASSWORD_CONFIRMED_ATTRIBS
        );
    ...
    {
        $input = new Input('raw_password', 'raw_password', '', [], static::RAW_PASSWORD_ATTRIBS);
    ...
    protected function addCanLogin(Entity $entity): User
    {
        ...
        $input = new Input('can_login', 'can_login', '1', [], $attributes);
    ...
    protected function addIsGravatarAllowed(Entity $entity): User
    {
        ...
        $input = new Input('is_gravatar_allowed', 'is_gravatar_allowed', '1', [], $attributes);
        $label = new Label('is_gravatar_allowed', 'admin:userIsGravatarAllowed');
    ...
    protected function addUserGroups(Entity $entity): User
    {
        ...
        $this->form[] = new FormGroup($this->createUserGroupSelect($options), $this->createUserGroupLabel());
    ...
    {
        $size = $this->getMultiSelectSize(count($options), static::MULTISELECT_MIN_SIZE, static::MULTISELECT_MAX_SIZE);

        $select = new MultiSelect('user_group_ids', 'user_group_ids[]', [], [Html5::ATTR_SIZE => [(string)$size]]);
        $select->add(...$options);
    ...
    protected function addUserLanguages(Entity $entity): User
    {
        ...
        $input   = $this->createUserLanguageSelect($options);
        $label   = $this->createUserLanguageLabel();
    ...
    protected function createUserLanguageSelect(array $options): Select
    {
        ...
        $select = new MultiSelect('user_language_id', 'user_language_id', [], [Html5::ATTR_SIZE => [(string)$size]]);
        $select->add(...$options);
