...
class Page extends Filters
{
    ...
    public function __construct(array $intents = [], array $attributes = [], ?string $tag = null)
    {
        ...
        $this
            ->add(new ExactFilter('identifier', 'website:pageIdentifier'))
            ->add(new LikeFilter('title', 'website:pageTitle'))
            ->add(new LikeFilter('lede', 'website:pageLede'))
            ->add(new LikeFilter('body', 'website:pageBody'));
    }
}
